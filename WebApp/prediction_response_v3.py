...
def send_notification(phone_number, message):
    ...
    twilio_phone_number = '+14025881918'
...
if 'trained_models' in st.session_state:
    ...
    if st.button("Predict"):
        ...
        phone_number = st.text_input("Enter phone number for notification (e.g., +1234567890):")
        if phone_number and st.button("Send Notification"):
            message_body = f"Disaster Prediction: {user_prediction[0]}\n{response_message}"
            notification_sid = send_notification(phone_number, message_body)
            st.write(f"Notification sent successfully with SID: {notification_sid}")

if __name__ == "__main__":
    st.sidebar.title("Disaster Prediction and Management")
    st.sidebar.info("Use the sidebar to navigate the application.")
