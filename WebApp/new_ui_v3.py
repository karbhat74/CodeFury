...
if section == "Disaster Prediction":
    ...
    <div style="background-color: #f9f9f9; padding: 20px; border-radius: 10px;">
        <h2 style="color: #009688;">Enter Disaster Details</h2>
    </div>
    """, unsafe_allow_html=True)
    ...
    if 'trained_models' in st.session_state:
        with st.form(key='prediction_form'):
            ...
            with col1:
                magnitude = st.number_input("Magnitude", min_value=0.0, value=7.2, step=0.1, format="%.1f", key="magnitude")
                total_deaths = st.number_input("Total Deaths", min_value=0, value=1500, step=10, key="total_deaths")
                no_injured = st.number_input("No. Injured", min_value=0, value=3000, step=10, key="no_injured")
                no_affected = st.number_input("No. Affected", min_value=0, value=50000, step=1000, key="no_affected")
                no_homeless = st.number_input("No. Homeless", min_value=0, value=10000, step=1000, key="no_homeless")

            with col2:
                disaster_group = st.selectbox("Disaster Group", options=['Natural', 'Technological'], key="disaster_group")
                disaster_subgroup = st.selectbox("Disaster Subgroup", options=['Geophysical', 'Hydrological', 'Meteorological', 'Climatological', 'Biological', 'Extra-terrestrial'], key="disaster_subgroup")
                disaster_type = st.selectbox("Disaster Type", options=['Earthquake', 'Volcanic activity', 'Mass movement (dry)', 'Storm', 'Flood', 'Mass movement (wet)', 'Wildfire', 'Extreme temperature', 'Drought', 'Glacial lake outburst', 'Insect infestation', 'Epidemic', 'Animal accident', 'Impact', 'Space weather'], key="disaster_type")
            ...
            if submit_button:
                ...
                model_name = st.selectbox("Select Model for Prediction", options=list(st.session_state['trained_models'].keys()), key="model_select")
...
# Section 2: Model Results
    ...
    if 'user_prediction' in st.session_state:
        ...
        st.write(f"### Predicted Disaster Subtype: {st.session_state['user_prediction']}")
